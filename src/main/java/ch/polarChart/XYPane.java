...
import static ch.polarChart.ChartType.SMOOTH_POLAR;
import static ch.polarChart.Helper.clamp;
...
public class XYPane<T extends XYItem> extends Region implements ChartArea {
    ...
    private void drawPolar(final XYSeries<T> SERIES) {
        ...
        double  phi      = Math.toRadians(clamp(0.0, 360.0, item.getX()));
        ...
        for (int i = 1 ; i < NO_OF_ITEMS  ;i++) {
            ...
            phi  = Math.toRadians(clamp(0.0, 360.0, item.getX()));
